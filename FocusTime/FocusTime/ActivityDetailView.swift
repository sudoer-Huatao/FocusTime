...
struct ActivityDetailView: View {
    ...
    // Get current app start time via a computed property
    private var currentAppStartTime: Date? {
        // Since currentAppStartTime is private in UsageTracker, we need a workaround
        // We'll create a method in UsageTracker to expose it, or track it differently
        // For now, we'll estimate based on when we last saw the current app
        return nil // We'll fix this properly below
    }
    
    // Filtered apps for selected date - FIXED
    private var filteredApps: [(String, TimeInterval)] {
        let apps: [(String, TimeInterval)]
        
        if isToday {
            // For today, get all apps from daily usage
            apps = usageTracker.getAllApps()
        } else {
            // For past dates, use historical data
            apps = usageTracker.getAppsForDate(selectedDate)
        }
    ...
    // Total usage for selected date - FIXED
    private var totalUsageForDate: TimeInterval {
        if isToday {
            // For today, use totalUsageToday
    ...
    private var emptyStateView: some View {
        VStack(spacing: 20) {
            if isToday && usageTracker.getAllApps().isEmpty {
            ...
            VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            GeometryReader { geometry in
                                ZStack(alignment: .leading) {
                                    ...
                                    Rectangle()
                                        .fill(Color.blue)
                                        .frame(width: geometry.size.width * CGFloat(min(totalUsageForDate / (24 * 3600), 1.0)), height: 6)
                                        .cornerRadius(3)
                                }
                            }
                            .frame(height: 6)
                ...
                VStack(alignment: .leading, spacing: 15) {
                    ...
                    ForEach(filteredApps, id: \.0) { app, duration in
                        AppUsageRow(appName: app, duration: duration, isToday: isToday, currentAppName: usageTracker.currentApp)
                    }
                }
                .padding(.horizontal)
            }
